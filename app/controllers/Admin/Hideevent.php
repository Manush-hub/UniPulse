...
class AdminHideevent extends Controller{
    ...
    public function index($eventId = '', $b = '' , $c = ''){
        ...
        if (!AuthService::isLoggedIn() || AuthService::getCurrentUser()['type'] !== 'admin') {
            ...
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                ...
                echo json_encode([
                    'success' => false,
                    'errors' => ['general' => 'You must be logged in as an admin to hide events. Please login and try again.'],
        ...
        // Only allow POST requests for hiding
        ...
        try {
            ...
            if ($result['success']) {
                ...
                if ($isAjax) {
                    ...
                    header('Location: /unipulse/public/admin/allevents?success=Event hidden successfully');
                    exit();
                }
            } else {
            ...
            error_log("Error in AdminHideevent::index: " . $e->getMessage());
            
            if ($isAjax) {
                echo json_encode([
                    'success' => false,
                    'errors' => ['general' => 'An error occurred while hiding the event. Please try again.']
                ]);
                exit();
            } else {
                header('Location: /unipulse/public/admin/allevents?error=An error occurred while hiding the event');
                exit();
            }
        }
    }
