...
// Compatibility layer migrated from extracted/Moderator_dashboard.js
...
function toggleUserReports() {
    ...
    hiddenRows.forEach(row => {
        ...
        row.style.display = shouldShow ? 'table-row' : 'none';
        if (icon) icon.style.transform = shouldShow ? 'rotate(180deg)' : 'rotate(0deg)';
        if (btnText) btnText.textContent = shouldShow ? 'Show Less' : 'View All Reports';
    });
}
...
function performApprovalAction(reason = '') {
    ...
    fetch(`/unipulse/public/moderator/publisherapproval/${endpoint}/${currentPublisherId}`, {
        ...
        .then(data => {
            if (!data.success) {
                throw new Error(data.message || `Failed to ${currentAction} publisher`);
            }
        ...
        .catch(error => {
            showNotification(error.message || `An error occurred while ${currentAction}ing the publisher`, 'error');
        });
}
...
function updateStats(action) {
    ...
    if (pendingElement) {
        const currentCount = parseInt(pendingElement.textContent, 10) || 0;
        pendingElement.textContent = Math.max(0, currentCount - 1);
    }

    const pendingBadge = document.querySelector('.stat-badge.pending');
    if (pendingBadge) {
        const currentPending = parseInt(pendingBadge.textContent.split(' ')[0], 10) || 0;
        pendingBadge.textContent = `${Math.max(0, currentPending - 1)} Pending`;
    }

    if (action === 'approve') {
        const approvedBadge = document.querySelector('.stat-badge.approved');
        if (approvedBadge) {
            const currentApproved = parseInt(approvedBadge.textContent.split(' ')[0], 10) || 0;
            approvedBadge.textContent = `${currentApproved + 1} Approved`;
        }
    } else if (action === 'reject') {
        const rejectedBadge = document.querySelector('.stat-badge.rejected');
        if (rejectedBadge) {
            const currentRejected = parseInt(rejectedBadge.textContent.split(' ')[0], 10) || 0;
...
document.addEventListener('click', function(e) {
    if (e.target.classList.contains('btn-approve') || e.target.closest('.btn-approve')) {
        ...
        approvePublisher(btn.dataset.publisherId);
    }
    ...
    if (e.target.classList.contains('btn-reject') || e.target.closest('.btn-reject')) {
        ...
        rejectPublisher(btn.dataset.publisherId);
    }
});
