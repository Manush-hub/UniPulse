...
try {
    ...
    try {
        ...
        $dataUpdates = [
            "UPDATE events SET visibility = 'public' WHERE visibility IN ('public', 'all-universities')",
            "UPDATE events SET visibility = 'university-only' WHERE visibility = 'university'",
            "UPDATE events SET visibility = 'faculty-only' WHERE visibility IN ('private', 'faculty')"
        ];
