    ...
    <script>
        (function() {
            ...
            const renderSummary = (summary) => {
                completedEventsCountEl.textContent = formatNumber(summary.events_count || 0);
                totalTicketsSoldEl.textContent = formatNumber(summary.tickets_count || 0);
                grossSalesTotalEl.textContent = formatCurrency(summary.gross_sales || 0);
                commissionTotalEl.textContent = formatCurrency(summary.commission_total || 0);
                profitTotalEl.textContent = formatCurrency(summary.profit_total || 0);
            };
                ...
                selectedEventDetailsEl.innerHTML = `
                    ...
                    <div class="selected-event-metrics">
                        <div class="selected-metric-card">
                            ...
                            <strong>${formatCurrency(selected.profit_total || 0)}</strong>
                        </div>
                        <div class="selected-metric-card">
                            <span>Total Tickets Sold</span>
                            <strong>${formatNumber(selected.total_tickets_sold || 0)}</strong>
                        </div>
                        <div class="selected-metric-card">
                            <span>Gross Sales</span>
                            <strong>${formatCurrency(selected.gross_sales || 0)}</strong>
                        </div>
                        <div class="selected-metric-card">
                            <span>Platform Commission</span>
                            <strong>${formatCurrency(selected.commission_total || 0)}</strong>
                        </div>
                        <div class="selected-metric-card">
                            <span>Free Tickets</span>
                            <strong>${formatNumber(selected.free_tickets || 0)}</strong>
                        </div>
                        <div class="selected-metric-card">
                            <span>Paid Tickets</span>
                            <strong>${formatNumber(selected.paid_tickets || 0)}</strong>
                        </div>
                    </div>
                        ...
                        <button type="button" class="event-profit-card ${isActive ? 'active' : ''}" data-event-id="${event.event_id}">
                            ...
                            <div class="event-profit-card-metrics">
                                <div>
                                    ...
                                    <strong>${formatNumber(event.total_tickets_sold || 0)}</strong>
                                </div>
                                <div>
                                    <small>Total Profit</small>
                                    <strong>${formatCurrency(event.profit_total || 0)}</strong>
                                </div>
                            </div>
                        </button>
                    `;
