...
class Event
{
     ...
     * 
     * Visibility Rules:
     * - faculty-only: Only visible to users from the same faculty AND university as the event organizer
     * - university-only: Only visible to users from the same university as the event organizer
     * - all-universities: Visible to all university users (anyone in university_users table)
     * - public: Visible to everyone (university users AND public users)
     */
    private function buildVisibilityFilter($currentUser = null)
    {
        ...
        // Public events - everyone can see (both university_user and public_user)
        $visibilityConditions[] = "e.visibility = 'public'";

        // University users can see more events based on their university/faculty
        if ($userType === 'university_user') {
            ...
            // University-only events - only users from the SAME university as event organizer
            if (!empty($userUniversity)) {
                ...
                // Faculty-only events - only users from the SAME faculty AND university as event organizer
        ...
        // Note: public_user type can only see public events (already added above)
