...
class Event
{
    ...
    public function getAllEvents($filters = [])
    {
        ...
        // Handle status filter based on event date, not stored status column
        ...
        // Sort by status first (ongoing, upcoming, completed), then by date
        $sql .= ' ORDER BY 
                    CASE 
                        WHEN DATE(e.event_date) = CURDATE() THEN 0
                        WHEN DATE(e.event_date) > CURDATE() THEN 1
                        ELSE 2
                    END ASC,
                    e.event_date ASC, 
                    e.event_time ASC';
    ...
    public function getEventsSeekingSponsors($filters = [])
    {
        ...
        $sql .= ' ORDER BY 
                    CASE 
                        WHEN DATE(event_date) = CURDATE() THEN 0
                        WHEN DATE(event_date) > CURDATE() THEN 1
                        ELSE 2
                    END ASC,
                    event_date ASC, 
                    event_time ASC';
    ...
    public function deleteEventAdmin($eventId)
    {
        try {
            ...
            $checkResult = $this->query($checkQuery, ['id' => $eventId]);
