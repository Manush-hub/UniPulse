...
class AdminShowevent extends Controller{
    ...
    public function index($eventId = '', $b = '' , $c = ''){
        ...
        if (!AuthService::isLoggedIn() || AuthService::getCurrentUser()['type'] !== 'admin') {
            ...
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                ...
                echo json_encode([
                    'success' => false,
                    'errors' => ['general' => 'You must be logged in as an admin to show events. Please login and try again.'],
        ...
        // Only allow POST requests for showing
        ...
        try {
            ...
            if ($result['success']) {
                ...
                if ($isAjax) {
                    ...
                    header('Location: /unipulse/public/admin/allevents?success=Event shown successfully');
                    exit();
                }
            } else {
            ...
            error_log("Error in AdminShowevent::index: " . $e->getMessage());
            
            if ($isAjax) {
                echo json_encode([
                    'success' => false,
                    'errors' => ['general' => 'An error occurred while showing the event. Please try again.']
                ]);
                exit();
            } else {
                header('Location: /unipulse/public/admin/allevents?error=An error occurred while showing the event');
                exit();
            }
        }
    }
