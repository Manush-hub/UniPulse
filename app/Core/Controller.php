...
class Controller
{
    ...
    /**
     * Load user profile photo and name into session for header display
     * Call this from any controller that needs to display the header with profile photo
     */
    protected function loadUserProfilePhotoToSession()
    {
        ...
        try {
            ...
            $profile = $model->find($currentUser['id']);
            if ($profile) {
                // Load profile photo
                if (!empty($profile->profile_photo)) {
                    $_SESSION['user_profile_photo'] = $profile->profile_photo;
                }
                // Load cover photo
                if (!empty($profile->cover_photo)) {
                    $_SESSION['user_cover_photo'] = $profile->cover_photo;
                }
                // Load full name for header
                if (!empty($profile->full_name)) {
                    $_SESSION['user_name'] = $profile->full_name;
                }
            }
        } catch (Exception $e) {
            error_log('Error loading profile data to session: ' . $e->getMessage());
        }
    }
}
